...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/dawn"
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case apitypes.ApplicationClique.Mime:
		...
		//TODO(keep)., a
	case apitypes.ApplicationDawn.Mime:
		stringData, ok := data.(string)
		if !ok {
			return nil, useEthereumV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationDawn.Mime)
		}
		dawnData, err := hexutil.Decode(stringData)
		if err != nil {
			return nil, useEthereumV, err
		}
		header := &types.Header{}
		if err := rlp.DecodeBytes(dawnData, header); err != nil {
			return nil, useEthereumV, err
		}
		// The incoming dawn header is already truncated, sent to us with a extradata already shortened
		...
		// Get back the rlp data, encoded by us
		sighash, dawnRlp, err := dawnHeaderHashAndRlp(header, api.chainID)
		if err != nil {
			return nil, useEthereumV, err
		}
		messages := []*apitypes.NameValueType{
		...
		useEthereumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: dawnRlp, Messages: messages, Hash: sighash}
...
func dawnHeaderHashAndRlp(header *types.Header, chainId *big.Int) (hash, rlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("dawn header extradata too short, %d < 65", len(header.Extra))
		return
	}
	rlp = dawn.DawnRLP(header, chainId)
	hash = dawn.SealHash(header, chainId).Bytes()
	return hash, rlp, err
}
